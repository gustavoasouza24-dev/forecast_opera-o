...
# app.py — Previsão de Entregas (SARIMAX) com upload CSV/Excel (data + 1 coluna de entregas)
...
def sniff_csv_from_bytes(content: bytes):
    ...
    Retorna (DataFrame, sep, encoding).
    """
    last_err = None
...
def read_excel_bytes(content: bytes, ext: str, sheet_name: str | None = None) -> tuple[pd.DataFrame, list[str]]:
    """
    Lê Excel de bytes. Se sheet_name=None, retorna somente a lista de abas.
    ...
    # reler porque já consumimos o ponteiro
...
def to_datetime_col(s: pd.Series):
    # tenta formatos comuns primeiro (pt-BR em prioridade), depois parse genérico
...
def make_br_holiday_set(year: int, include_facultativos: bool, include_joinville: bool) -> set[str]:
    ...
    good_friday = (pascoa - pd.Timedelta(days=2)).strftime("%Y-%m-%d")
    S.add(good_friday)  # Sexta-feira Santa

    if include_facultativos:
        carnaval_seg = (pascoa - pd.Timedelta(days=48)).strftime("%Y-%m-%d")
        carnaval_ter = (pascoa - pd.Timedelta(days=47)).strftime("%Y-%m-%d")
        cinzas = (pascoa - pd.Timedelta(days=46)).strftime("%Y-%m-%d")
        corpus = (pascoa + pd.Timedelta(days=60)).strftime("%Y-%m-%d")
        S.update({carnaval_seg, carnaval_ter, cinzas, corpus})
...
# =========================
# Upload CSV/Excel e mapeamento de colunas
# =========================
st.title("Previsão de Entregas (SARIMAX)")
st.write("Envie um **CSV** ou **Excel** com **uma coluna de data** e **uma coluna de entregas**.")
...
if name.endswith(".xlsx") or name.endswith(".xls"):
    ...
    try:
        ...
        st.error(f"Erro ao ler a planilha '{sheet}': {e}")
        st.stop()
...
# 2) CSV?
    ...
    st.error("Formato não suportado. Envie .csv, .xlsx ou .xls.")
    st.stop()
...
# Detectar colunas de data candidatas
date_candidates = []
...
for cand in ("data", "date", "dt"):
    if cand in [c.lower() for c in raw_df.columns]:
        ...
        default_date_col = next(col for col in raw_df.columns if col.lower() == cand)
        break
...
if dt_series.isna().all():
    st.error(f"A coluna '{date_col}' não pôde ser convertida em datas.")
    st.stop()

# Coerção numérica para demais colunas → detectar entregas
...
if not numeric_cols:
    st.error("Não encontrei nenhuma coluna numérica para entregas. Confira seu arquivo.")
    st.stop()

default_target = numeric_cols[0] if len(numeric_cols) == 1 else None
target_col = st.selectbox("Selecione a coluna de **entregas**", options=numeric_cols,
                          index=(numeric_cols.index(default_target) if default_target in numeric_cols else 0))

# Base diária
df = pd.DataFrame({"data": dt_series, "entregas": num_df[target_col]})
df = df.dropna(subset=["data"]).sort_values("data").set_index("data").asfreq("D")
df["entregas"] = df["entregas"].fillna(method="ffill").fillna(0)
...
exog_train_base = pd.concat([df["is_weekend"], is_holiday_train], axis=1)
...
if do_backtest and st.button("📏 Rodar Backtest"):
    ...
    if bt_start <= df.index.min():
        ...
        series_train = prepare_series(train_end, df["entregas"])
        ...
        try:
            mean_bt, li_bt, ls_bt = fit_and_forecast(
                series_train, exog_tr, exog_bt,
                order=(p, d, q), seasonal_order=(P, D, Q, s)
            )
        except Exception as e:
            st.error(f"Falha no backtest: {e}")
        else:
            real_bt = df["entregas"].reindex(bt_index).fillna(0)
            mape = (np.abs((real_bt - mean_bt) / np.where(real_bt==0, np.nan, real_bt))).mean() * 100
            rmse = np.sqrt(((real_bt - mean_bt) ** 2).mean())
            st.write(pd.DataFrame({"MAPE (%)": [round(mape,2)], "RMSE": [round(rmse,2)]}))
            ...
            ax.set_title(f"Backtest — {bt_days} dias")
...
if st.button("🚀 Gerar Previsão", type="primary"):
    series_train = prepare_series(last_date, df["entregas"])
    exog_tr = exog_train_base.reindex(series_train.index)
    ...
    try:
        ...
        st.error(f"Falha ao ajustar/prever: {e}")
    else:
        warnings.filterwarnings("default")
        # Tabela e gráfico
        out = pd.DataFrame({
            "Previsao": pred_mean.round().astype(int),
            "LI_95%": ci_lower.round().astype(int),
            "LS_95%": ci_upper.round().astype(int)
        ...
        real_data = df["entregas"].reindex(forecast_index)
        if real_data.notna().any():
            real_data.plot(ax=ax, label="Real (período previsto)", color="orange", linewidth=2)
        ax.set_title("SARIMAX — Entregas")
        ax.grid(True); ax.legend()
        st.pyplot(fig)

        # Download CSV
        csv_bytes = out.to_csv(sep=";", decimal=",", index_label="data").encode("utf-8-sig")
        st.download_button(
            "💾 Baixar CSV da previsão",
            data=csv_bytes,
            file_name="previsao_entregas.csv",
            mime="text/csv",
